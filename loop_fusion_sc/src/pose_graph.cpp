...
//            if (ret[i].Score > 0.015)
    ...
    if (DEBUG_IMAGE)
    {
//        cv::imwrite(VINS_RESULT_PATH_BASE+"/matching/match_"+to_string(keyframe->time_stamp*1e9)+".png",loop_result);
        ...
        if (cur_index != -1)
        {
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                if((*it)->has_loop)
                {
                    ...
                    if(SWITCHABLE_ON)
                    {
                      swc_array[i][0] = (*it)->loop_swc;
                      ceres::CostFunction* cost_function = FourDOFWeightErrorSWC::Create( relative_t.x(), relative_t.y(), relative_t.z(),
                                                                                 relative_yaw, euler_conncected.y(), euler_conncected.z());
                      problem.AddResidualBlock(cost_function, loss_function, euler_array[connected_index],
                                                                    ...
                                                                    t_array[i],
                                                                    swc_array[i]);
                      ceres::CostFunction* cost_function1 = PriorFunctor::Create(SWITCHABLE_GAMMA);
                      problem.AddResidualBlock(cost_function1,NULL,swc_array[i]);
                      problem.SetParameterLowerBound(swc_array[i],0,0);
                      problem.SetParameterUpperBound(swc_array[i],0,1);
...
//            printf("optimize cost: %f,%d \n", t_optimize.toc(),cur_index);
            ...
            */
            ...
            int undr = 0;
            int over = 0;
            if(SWITCHABLE_ON) std::cout<<"VINS WITH SC WEIGHT"<<std::endl;
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                if ((*it)->index < first_looped_index)
                    continue;
                ...
                if((*it)->has_loop && SWITCHABLE_ON)
                {
                  if(swc_array[i][0]<0.5) undr++;
                  else over++;
                  (*it)->loop_swc = swc_array[i][0];
                }
            ...
            if(SWITCHABLE_ON)
            {
              std::cout<<"OVER 0.5 : "<<over<<std::endl;
              std::cout<<"UNDR 0.5 : "<<undr<<std::endl;
            }
...
//            printf("optimize pose graph \n");
            ...
            for (it = keyframelist.begin(); it != keyframelist.end(); it++)
            {
                ...
                if((*it)->has_loop)
                {
                    ...
                    {
                      swc_array[i][0] = (*it)->loop_swc;
                      ceres::CostFunction* loop_function = RelativeRTErrorSWC::Create(relative_t.x(), relative_t.y(), relative_t.z(),
                                                                                  relative_q.w(), relative_q.x(), relative_q.y(), relative_q.z(),
                                                                                  0.1, 0.01);
                      problem.AddResidualBlock(loop_function, loss_function, q_array[connected_index], t_array[connected_index],
                                               q_array[i], t_array[i],
                                               swc_array[i]);
                      ceres::CostFunction* cost_function1 = PriorFunctor::Create(SWITCHABLE_GAMMA);
                      problem.AddResidualBlock(cost_function1,NULL,swc_array[i]);
...
void PoseGraph::updatePath()
{
    ...
    if (SAVE_LOOP_PATH)
    {
        ...
        ofstream loop_path_file_tmp0(VINS_RESULT_PATH_BASE+"/loopinfo.csv", ios::out);
        loop_path_file_tmp0.close();

        ofstream loop_path_file_tmp1(VINS_RESULT_PATH_BASE+"/results/VINSFUSION_KF_"+
                                    to_string((keyframelist.back()->time_stamp*1e9))+".csv", ios::out);
        loop_path_file_tmp1.close();
    }

    ofstream loop_closure_file(VINS_RESULT_PATH_BASE+"/results/VINSFUSION_LOOP_"+
                            to_string((keyframelist.back()->time_stamp*1e9))+".csv", ios::out);
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
        ...
        if (SAVE_LOOP_PATH)
        {
            ...
            if((*it)->has_loop)
            {
              ofstream loop_path_file0(VINS_RESULT_PATH_BASE+"/loopinfo.csv", ios::app);
              loop_path_file0.setf(ios::fixed, ios::floatfield);
              loop_path_file0 << (*it)->time_stamp<< " "
                             << getKeyFrame((*it)->loop_index)->time_stamp<< " ";
              loop_path_file0.precision(6);
              loop_path_file0 << (*it)->loop_swc<<endl;
              loop_path_file0.close();
            }

            ofstream loop_path_file1(VINS_RESULT_PATH_BASE+"/results/VINSFUSION_KF_"+
                                        to_string((keyframelist.back()->time_stamp*1e9))+".csv", ios::app);
            loop_path_file1.setf(ios::fixed, ios::floatfield);
            loop_path_file1.precision(9);
            loop_path_file1 << (*it)->time_stamp<< " ";
            loop_path_file1.precision(6);
            loop_path_file1 << P.x() << " "
                           << P.y() << " "
                           << P.z() << " "
                           << Q.w() << " "
        ...
        if (SHOW_L_EDGE)
        {
            if ((*it)->has_loop && (*it)->sequence == sequence_cnt)
            {
                ...
                loop_closure_file << (*it)->time_stamp*1e9 << " "<< connected_KF->time_stamp*1e9<<endl;
            }
        }
...
{
  ofstream loop_path_file1(VINS_RESULT_PATH_BASE+"/results/"+"vinsfusion_KF_"+to_string((keyframelist.back()->time_stamp*1e9))+".csv", ios::out);
  loop_path_file1.close();
  list<KeyFrame*>::iterator it;
  for (it = keyframelist.begin(); it != keyframelist.end(); it++)
  {
    ...
    if (SAVE_LOOP_PATH)
    {
      ...
      ofstream loop_path_file1(VINS_RESULT_PATH_BASE+"/results/"+"vinsfusion_KF_"+to_string((keyframelist.back()->time_stamp*1e9))+".csv", ios::app);
      loop_path_file1.setf(ios::fixed, ios::floatfield);
      loop_path_file1.precision(9);
      loop_path_file1 << (*it)->time_stamp<< " ";
      loop_path_file1.precision(6);
